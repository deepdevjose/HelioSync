...
function getPanelBehavior(panel, t) {
  const mode = `${panel.tracking_mode || ''}`.trim().toUpperCase();
  const angleError = Math.abs(panel.angle_error_deg);

  if (mode === 'STATIC') {
    return {
      label: t('insights.fixedOrientation'),
      detail: t('insights.recommendedAlignmentApplied'),
    };
  }

  if (panel.servo_active || angleError > 2) {
    return {
      label: t('insights.repositioning'),
      detail: t('solar.optimalAngle'),
    };
  }

  if (angleError > 0.6) {
    return {
      label: t('insights.trackingActive'),
      detail: t('solar.capturingEfficiently'),
    };
  }

  return {
    label: t('solar.aligned'),
    detail: t('solar.alignedWithSun'),
  };
}
...
export default function SystemViewCard() {
  ...
  const panelBehavior = useMemo(() => getPanelBehavior(data.panel, t), [data.panel, t]);
  const alignmentPercent = Math.round(Math.max(0, Math.min(1, insights.outputState.ratio)) * 100);

  return (
    <GlassCard delay={0.1} className="flex h-full min-h-[520px] min-w-0 flex-col gap-5 overflow-hidden !p-6 md:!p-7">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div className="space-y-2">
          ...
          <h2 className="m-0 text-3xl font-semibold tracking-[-0.03em] text-white">{t('dashboard.liveSolarBehavior')}</h2>
        ...
        <div className="flex flex-wrap items-start gap-3">
          <div className="rounded-2xl border border-white/[0.08] bg-black/10 px-4 py-3">
            <div className="text-[11px] uppercase tracking-[0.24em] text-slate-500">{t('dashboard.panelState')}</div>
            <div className="mt-2 text-base font-medium text-white">{panelBehavior.label}</div>
            <div className="mt-1 text-sm text-slate-400">{panelBehavior.detail}</div>
          </div>

          <div className="rounded-2xl border border-white/[0.08] bg-black/10 px-4 py-3">
            <div className="text-[11px] uppercase tracking-[0.24em] text-slate-500">{t('dashboard.outputRead')}</div>
            <div className="mt-2 text-base font-medium text-white">{insights.outputState.label}</div>
            <div className="mt-1 text-sm text-slate-400">{insights.systemState.label}</div>
          </div>

          <button
            type="button"
            onClick={() => setExpanded((value) => !value)}
            className="inline-flex h-11 w-fit items-center gap-2 rounded-full border border-white/10 bg-white/[0.06] px-4 py-2 text-sm font-medium text-white transition hover:bg-white/10"
          >
      ...
      {expanded ? (
        <>
          ...
          <div className="grid gap-3 lg:grid-cols-3">
            <div className="rounded-2xl border border-white/[0.08] bg-black/10 px-4 py-3">
              ...
              <div className="mt-1 text-sm text-slate-400">{panelBehavior.label}</div>
            </div>
        ...
        <div className="flex min-h-[220px] flex-1 items-center justify-center rounded-[28px] border border-dashed border-white/10 bg-black/10 px-6 text-center text-sm leading-6 text-slate-400">

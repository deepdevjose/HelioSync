...
function AuthSolarTrajectory({ t }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-[32px] border border-white/[0.08] bg-[linear-gradient(180deg,rgba(8,12,18,0.78),rgba(12,18,30,0.42))] px-5 py-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
      ...
      <div className="relative z-10 flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-1 rounded-full border border-white/10 bg-black/28 p-1">
          ...
          <div className="inline-flex items-center gap-2 rounded-full border border-helium-300/22 bg-helium-300/12 px-3 py-1 text-[10px] font-medium uppercase tracking-[0.16em] text-helium-100 shadow-[0_0_18px_rgba(56,189,248,0.12)]">
      ...
      <div className="relative mt-4 overflow-hidden rounded-[28px] border border-white/[0.08] bg-[linear-gradient(180deg,rgba(7,11,19,0.78),rgba(8,14,24,0.38))] px-3 py-3">
        <svg className="pointer-events-none h-[180px] w-full" viewBox="0 0 360 180" fill="none" preserveAspectRatio="none" aria-hidden="true">
          <defs>
        ...
        {labels.map((item) => (
          <div
            key={item.key}
            className={`absolute rounded-full border px-2.5 py-1 text-[10px] font-medium uppercase tracking-[0.16em] shadow-[0_10px_20px_rgba(0,0,0,0.18)] ${item.className}`}
            style={{
      ...
      <div className="relative z-10 mt-3 grid gap-2 sm:grid-cols-3">
        {[
          ...
          <div key={metric.label} className="rounded-[20px] border border-white/[0.08] bg-black/18 px-3 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.02)]">
            <div className="text-[10px] font-medium uppercase tracking-[0.18em] text-slate-500">{metric.label}</div>
...
export default function AuthShell({ eyebrow, title, subtitle, children, footer }) {
  ...
  return (
    <div className="min-h-screen glass-mesh-bg px-4 py-6 md:px-8 md:py-8">
      <div className="mx-auto grid min-h-[calc(100vh-3rem)] max-w-7xl gap-6 xl:grid-cols-[0.92fr_1.08fr]">
        <GlassCard delay={0.05} className="relative flex min-h-[300px] flex-col justify-between gap-6 overflow-hidden border-white/[0.07] bg-slate-950/66 !p-7 shadow-[0_28px_70px_rgba(0,0,0,0.26)] md:!p-8">
          ...
          <div className="relative z-10 space-y-6">
            ...
            <div className="space-y-5">
              ...
              <h1 className="m-0 max-w-xl text-[2.65rem] font-semibold tracking-[-0.06em] text-white md:text-[4.2rem] md:leading-[0.95]">
                {t('authShell.heroTitle')}
              </h1>

              <p className="m-0 max-w-2xl text-base leading-7 text-slate-300/88 md:text-[1.02rem]">
          ...
          <div className="relative z-10 grid gap-2.5 md:grid-cols-3">
            {productPoints.map((point) => {
              const Icon = point.icon;

              return (
                <div key={point.title} className="rounded-[26px] border border-white/[0.08] bg-[linear-gradient(180deg,rgba(255,255,255,0.03),rgba(0,0,0,0.16))] p-3.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]">
                  ...
                  <div className="mt-3.5 text-[0.95rem] font-medium text-white">{point.title}</div>
        ...
        <div className="flex items-center xl:pl-2">
          <GlassCard delay={0.12} className="relative w-full overflow-hidden border-helium-400/18 bg-slate-950/88 !p-7 shadow-[0_34px_90px_rgba(0,0,0,0.34),0_0_0_1px_rgba(125,211,252,0.05)] md:!p-8">
            ...
            <div className="relative z-10 space-y-2">
              ...
              <h2 className="m-0 text-3xl font-semibold tracking-[-0.05em] text-white md:text-[2.2rem]">{title}</h2>
